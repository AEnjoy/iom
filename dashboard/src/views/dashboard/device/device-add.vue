...
const submitForm = async (values) => {
  let token,gid
  if(values.name===''){
    ElMessageBox.alert("设备名不能为空",'注意',{confirmButtonText: 'OK'})
    return
  }
  ...
  console.log(values)
    ...
    let obj= await axios.get("/api/group/search/name-get-id?name="+values.group.value)
    if(obj.status===200){
      gid=obj.data
    }
    let params = new URLSearchParams();
    params.append('deviceName', values.name.value);
    params.append('deviceFlag', values.type.value);
    params.append('groupId', gid);
    params.append('devicesToken',token);
    params.append('devicesWeight', values.weight.value);
    params.append('deviceId',0)
    let data= await axios.post('/api/devices/add', params)
  if(data.status===200){
    ElMessageBox.alert("设备添加成功",'提示',{confirmButtonText: 'OK'})
    //resetForm(form)
  }else{
    ElMessageBox.alert("设备添加失败.错误信息:"+data.data,'提示',{confirmButtonText: 'OK'})
  }
}
...
const rules = reactive({
  ...
  weight:[
    { required: true, message: '请输入设备权重', trigger: 'blur' }]
})
